...
function SearchBar() {
    ...
    function handleSelectedProduct(searchTerm) {
        if (searchTerm) {
            navigate(selectedCategory == "all"
                ...
                : `/?catefory=${selectedCategory}&searchterm=${searchTerm}`
            );
        } else {
    ...
    return (
        <Search sx={{ mr: 0 }}>
            <Select
                value={selectedCategory} size="small" sx={{
                    m: 1, "&": {
                        "::before": { ":hover": { border: "none", }, },
                        "::before, &::after": {
                            border: "none",
                        },
            ...
            <StyleAutocomplete freeSolo id="selected-product" disablePortal
                options={Array.from(
...
export default function Header() {
    ...
    const renderMenu = (
        <Menu anchorEl={anchorEl} id='user-profile-menu' keepMounted transformOrigin={{ horizontal: "right", vertical: "top" }}
            anchorOrigin={{ horizontal: 'right', vertical: 'bottom' }} open={isMenuOpen} onClose={handleMenuClose}>
            ...
            <AppBar position='sticky' sx={{ py: 1 }}>
                <Toolbar sx={{ display: "flex", gap: 2 }}>
                    <Typography variant='h6' color="inherit" fontSize="24px">
                        <StyledLink to="/">E-com</StyledLink>
                    </Typography>
                    <SearchBar />
                    <Box sx={{ display: { sx: "none", md: "flex" } }} >
                        <IconButton size="large" aria-laber="shows number of items added to cart" color="inherit" onClick={navigateToCart}>
                            <Badge badgeContent={count} color="error">
                                <ShoppingCartIcon />
                            </Badge>
                        </IconButton>
                        {user ? (
                            <Button onClick={handleProfileMenuOpen} color="inherit" sx={{ width: theme.spacing(20) }}>
